...
//		mMapView = new MapView(this);
		...
		curBusDeviceInfo = (BusDeviceInfo) intent.getSerializableExtra("DevObj");
	 ...
	 * @param context
	 */
	private void registErrorReciver(Context context) {
	...
	private Handler handler3 = new Handler() {
		public void handleMessage(Message msg) {
			...
			myGeoPoint = new GeoPoint((int) (latitude * 1E6), (int) (longitude * 1E6));
			...
			new Thread(new Runnable() {
				...
				public void run() {
					...
					try {
						...
						Map<String, String> maps = GpsToBaiduUtil.ConvertGpsToBaidu(guid_tmp, latitude, longitude);
	...
	private Handler handler2 = new Handler() {
		...
		public void handleMessage(Message msg) {
			...
			bundle = msg.getData();
			currentPoint = new GeoPoint((int) (bundle.getDouble("lat") * 1e6), (int) (bundle.getDouble("lon") * 1e6));
			if (isFirstGetGps) {// 首次获取设备GPS
				// mapView.getOverlays().clear();
				pt1 = new GeoPoint((int) (bundle.getDouble("lat") * 1e6), (int) (bundle.getDouble("lon") * 1e6));
				isFirstGetGps = false;
			} else {
				//
				pt2 = new GeoPoint((int) (bundle.getDouble("lat") * 1e6), (int) (bundle.getDouble("lon") * 1e6));
				// 画轨迹
				// GraphicsOverlay graphicsOverlay = new
				// GraphicsOverlay(mapView);
				...
				// if(ptLast != null){
				// // 画旧新交接轨迹
				// GraphicsOverlay graphicsOverlay_tmp = new
				// GraphicsOverlay(mapView);
				...
				// // 生成Graphic对象
				// Graphic lineGraphic_tmp = new Graphic(lineGeometry_tmp,
				// lineSymbol_tmp);
	...
	private boolean validateGps() {
		if (curBusDeviceInfo != null && 0d != curBusDeviceInfo.getLatitude() && 0d != curBusDeviceInfo.getLongitude()) {
			return true;
		} else {
			return false;
		}
